...
from crypto_db.models import (
    CryptoTransaction, FiatDeposit, TaxReport,
...
class SpanishTaxService:
    ...
    def calculate_capital_gains(
        ...
        ).order_by(CryptoTransaction.transaction_date).all()
        ...
        return {
            ...
            'remaining_holdings': {
                crypto: sum(h['amount'] for h in lots)
                for crypto, lots in holdings.items()
                if lots
            }
        }
    ...
    def generate_tax_report(
        ...
        fiat_deposits = db.query(FiatDeposit).filter(
            FiatDeposit.transaction_date >= start_date,
            FiatDeposit.transaction_date <= end_date
        ).all()
        
        fiat_summary = {
            'total_deposits': sum(
                d.amount for d in fiat_deposits
                if d.deposit_type == DepositType.DEPOSIT
            ),
            'total_withdrawals': sum(
                d.amount for d in fiat_deposits
                if d.deposit_type == DepositType.WITHDRAWAL
            )
        }
