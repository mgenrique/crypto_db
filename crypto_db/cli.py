...
@click.option(
    ...
    required=True,
    type=click.Choice(
        ["buy", "sell", "transfer_in", "transfer_out", "stake", "unstake", "reward", "fee"]
    ),
)
...
def add_crypto_transaction(
    platform_id, tx_type, crypto, amount, price, currency, fee, date, tx_id, notes
):
...
@click.option("--platform-id", "-p", required=True, type=int, help="Platform ID")
@click.option(
    "--type", "-t", "deposit_type", required=True, type=click.Choice(["deposit", "withdrawal"])
)
